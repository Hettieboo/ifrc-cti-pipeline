...
with col2:
    ...
    sentiment_counts = qual["sentiment_label"].value_counts().reset_index()
    sentiment_counts.columns = ["sentiment","count"]
    fig_sentiment = px.pie(sentiment_counts, values="count", names="sentiment",
        color="sentiment", color_discrete_map={"positive":"#2E8B57","neutral":"#FF8C00","negative":"#C8102E"}, hole=0.4)
    fig_sentiment.update_layout(height=200, margin=dict(l=0,r=0,t=20,b=0),
        legend=dict(orientation="h",yanchor="bottom",y=-0.2))
    st.plotly_chart(fig_sentiment, use_container_width=True)
    theme_counts = qual["theme_label"].value_counts().head(5).reset_index()
    theme_counts.columns = ["theme","count"]
    fig_themes = px.bar(theme_counts, x="count", y="theme", orientation="h",
        color="count", color_continuous_scale=["#C8102E","#FF8C00","#2E8B57"],
        labels={"count":"Mentions","theme":""})
    fig_themes.update_layout(height=190, margin=dict(l=0,r=0,t=10,b=0), showlegend=False, coloraxis_showscale=False)
...
st.markdown("---")
st.markdown('<div style="text-align:center;color:#999;font-size:12px;">🔴 IFRC Community Trust Index Dashboard | Henrietta Atsenokhai<br>Bronze → Silver → Gold Lakehouse | Python · dbt · Streamlit · Folium · Plotly · Esri Satellite Imagery</div>', unsafe_allow_html=True)
